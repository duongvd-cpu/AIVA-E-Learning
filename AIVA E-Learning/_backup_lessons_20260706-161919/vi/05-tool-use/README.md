...
- Những cân nhắc đặc biệt nào khi sử dụng Mô Hình Thiết Kế Sử Dụng Công Cụ để xây dựng các Agent AI đáng tin cậy?
...
- Nhận biết các cân nhắc để đảm bảo sự đáng tin cậy trong các Agent AI sử dụng mô hình thiết kế này.
...
> [!TIP]
> **Nhìn vào Code Thực Tế: Tool Use hoạt động ra sao?**  
> Trong dự án `my-practice/web-agent/`, bạn có thể mở file `skills/weather_skill.py` để xem một ví dụ kinh điển về **Tool Use**. 
> - Chúng ta định nghĩa một Python function `get_weather(location)`.
> - Chúng ta đăng ký nó với LLM (thông qua mảng `tools=[get_weather]`).
> - Khi người dùng hỏi thời tiết, LLM không trả lời bừa, nó sẽ yêu cầu (call) hàm này để lấy dữ liệu thực tế trước rồi mới phản hồi.

---

## Mô hình thiết kế sử dụng tool là gì?

**Mô Hình Thiết Kế Sử Dụng Công Cụ** tập trung vào việc cung cấp cho LLM khả năng tương tác với các công cụ bên ngoài để đạt được các mục tiêu cụ thể. Công cụ là mã có thể được thực thi bởi Agent để thực hiện các hành động. Một công cụ có thể là một hàm đơn giản như máy tính, hoặc một gọi API tới dịch vụ bên thứ ba như tra cứu giá cổ phiếu hoặc dự báo thời tiết. Trong bối cảnh Agent AI, các công cụ được thiết kế để được thực thi bởi Agent nhằm phản hồi các **gọi hàm do mô hình tạo ra**.

## Các trường hợp sử dụng có thể áp dụng là gì?

Các Agent AI có thể tận dụng công cụ để hoàn thành các tác vụ phức tạp, lấy thông tin hoặc đưa ra quyết định. Mô hình thiết kế sử dụng tool thường được sử dụng trong các tình huống đòi hỏi tương tác động với các hệ thống bên ngoài, chẳng hạn như cơ sở dữ liệu, dịch vụ web hoặc trình thông dịch mã. Khả năng này hữu ích cho một số trường hợp sử dụng khác nhau bao gồm:

- **Truy xuất thông tin động:** Agent có thể truy vấn API bên ngoài hoặc cơ sở dữ liệu để lấy dữ liệu cập nhật (ví dụ: truy vấn cơ sở dữ liệu SQLite để phân tích dữ liệu, lấy giá cổ phiếu hoặc thông tin thời tiết).
- **Thực thi và thông dịch mã:** Agent có thể thực thi mã hoặc tập lệnh để giải các bài toán toán học, tạo báo cáo hoặc thực hiện mô phỏng.
...
- **Hỗ trợ khách hàng:** Agent có thể tương tác với hệ thống CRM, nền tảng quản lý vé hoặc cơ sở kiến thức để giải quyết các câu hỏi của người dùng.
- **Tạo và chỉnh sửa nội dung:** Agent có thể sử dụng các công cụ như kiểm tra ngữ pháp, tóm tắt văn bản hoặc đánh giá an toàn nội dung để hỗ trợ các nhiệm vụ tạo nội dung.

## Các thành phần/khối xây dựng cần thiết để triển khai mô hình thiết kế sử dụng tool là gì?

Các khối xây dựng này cho phép Agent AI thực hiện nhiều loại nhiệm vụ khác nhau. Hãy xem xét các thành phần chính cần thiết để triển khai Mô Hình Thiết Kế Sử Dụng Công Cụ:
...
- **Logic Thực thi Hàm**: Quy định cách và thời điểm các công cụ được gọi dựa trên ý định người dùng và ngữ cảnh cuộc hội thoại. Điều này có thể bao gồm các module lập kế hoạch, cơ chế định tuyến hoặc luồng điều kiện quyết định việc sử dụng tool một cách linh hoạt.
...
- **Khung Tích hợp Công cụ**: Hạ tầng kết nối Agent với các công cụ khác nhau, dù là các hàm đơn giản hay dịch vụ bên ngoài phức tạp.
...
Để các nhà phát triển triển khai gọi hàm cho Agent, bạn sẽ cần:
...
Gọi Hàm là trung tâm của hầu hết, nếu không muốn nói là tất cả, các thiết kế sử dụng tool Agent, tuy nhiên việc triển khai từ đầu đôi khi có thể là thách thức.
Như chúng ta đã học trong [Bài học 2](../../../03-explore-agentic-frameworks) các khung agentic cung cấp các khối xây dựng sẵn để triển khai việc sử dụng tool.
...
<a href="https://learn.microsoft.com/azure/ai-services/agents/overview" target="_blank">Khung Đại Lý Microsoft</a> là một khung AI mã nguồn mở để xây dựng các Agent AI. Nó đơn giản hóa quá trình gọi hàm bằng cách cho phép bạn định nghĩa công cụ dưới dạng các hàm Python với decorator `@tool`. Khung này xử lý giao tiếp qua lại giữa mô hình và mã của bạn. Nó cũng cung cấp truy cập tới các công cụ dựng sẵn như Tìm tập tin và Trình thông dịch Mã qua `AzureAIProjectAgentProvider`.
...
<a href="https://learn.microsoft.com/azure/ai-services/agents/overview" target="_blank">Dịch Vụ Đại Lý Azure AI</a> là một khung agentic mới hơn được thiết kế nhằm giúp các nhà phát triển xây dựng, triển khai và mở rộng quy mô Agent AI chất lượng cao một cách bảo mật mà không cần quản lý tài nguyên tính toán và lưu trữ bên dưới. Nó đặc biệt hữu ích cho các ứng dụng doanh nghiệp vì là dịch vụ được quản lý hoàn toàn với bảo mật cấp doanh nghiệp.
...
Hãy tưởng tượng bạn là Agent bán hàng tại một công ty có tên Contoso. Bạn muốn phát triển một Agent đàm thoại có thể trả lời các câu hỏi về dữ liệu bán hàng của mình.
...
## Những cân nhắc đặc biệt khi sử dụng Mô Hình Thiết Kế Sử Dụng Công Cụ để xây dựng Agent AI đáng tin cậy?
...
[Hiểu về Các Mô Hình Thiết Kế Agentic](../04-agentic-design-patterns/README.md)

## Bài học tiếp theo
[Agentic RAG](../06-agentic-rag/README.md)
